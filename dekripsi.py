...
from tkinter import (
    ...
    StringVar, OptionMenu, Canvas, Frame, Scrollbar, BooleanVar, Checkbutton
)
...
def load_original_image():
    ...
    info_before.config(text=f"Gambar Asli - Ukuran: {img.width}x{img.height} | Format: {img.format or 'N/A'} | Mode: {img.mode}")
...
def decrypt():
    try:
        ...
        if mode_var.get() == "Teks":
            ...
            info_after.config(text=f"Hasil Dekripsi - Ukuran: {img.width}x{img.height} | Format: {img.format or 'N/A'} | Mode: {img.mode}")
...
Checkbutton(scroll_frame, text="Simulasikan Tag Poly1305 Invalid", variable=simulate_invalid).pack()

Label(scroll_frame, text="Nonce (Base64):").pack()
entry_nonce = Entry(scroll_frame, width=100)
entry_nonce.pack(pady=5)

Label(scroll_frame, text="Tag Poly1305 (Base64):").pack()
entry_tag = Entry(scroll_frame, width=100)
